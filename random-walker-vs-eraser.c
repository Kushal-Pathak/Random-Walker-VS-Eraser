...
void walk(float speed){
	random(speed);
	switch(wdir){
		case 0: wy=(wy+h-1)%h; walker=up; break;
		case 1: wx=(wx+1)%w; walker=right; break;
		case 2: wy=(wy+1)%h; walker=down; break;
		case 3: wx=(wx+w-1)%w; walker=left; break;
	}
	switch(edir){
		case 0: ey=(ey+h-1)%h; break;
		case 1: ex=(ex+1)%w; break;
		case 2: ey=(ey+1)%h; break;
		case 3: ex=(ex+w-1)%w; break;
	}
	matrix[wy][wx]=path;
		...
		for(j=0;j<w;j++){
			if(j==wx&&i==wy||j==ex&&i==ey){
				if(ex==wx&&ey==wy)
					...
					if(j==wx&&i==wy)printf("%c ", walker);
					if(j==ex&&i==ey)printf("%c ", eraser);
				}
			}else{
				printf("%c ", matrix[i][j]);
			}	
